 ...
 * @retval      无
 */
uint16_t g_flashbuf[FMC_SECTOR_SIZE / 2]; /* 最多是2K字节 */
void stmflash_write(uint32_t waddr, uint16_t *pbuf, uint16_t length)
{
    ...
    if (waddr < FMC_FLASH_BASE || waddr % 4 || (waddr + length) >= FMC_FLASH_END)
    {
        return; /* 非法地址 */
    }
    ...
    offaddr = waddr - FMC_FLASH_BASE;       /* 实际偏移地址. */
    ...
    while (1)
    {
        stmflash_read(secpos * FMC_SECTOR_SIZE + FMC_FLASH_BASE, g_flashbuf, FMC_SECTOR_SIZE / 2); /* 读出整个扇区的内容 */
        ...
        if (i < secremain) /* 需要擦除 */
        {
            ...
            flash_eraseop.PageAddress = secpos * FMC_SECTOR_SIZE + FMC_FLASH_BASE;  /* 要擦除的扇区 */
            ...
            stmflash_write_nocheck(secpos * FMC_SECTOR_SIZE + FMC_FLASH_BASE, g_flashbuf, FMC_SECTOR_SIZE / 2); /* 写入整个扇区 */
        }
        else
        {
        ...
        else       /* 写入未结束 */
        {
            ...
            if (length > (FMC_SECTOR_SIZE / 2))
            {
                secremain = FMC_SECTOR_SIZE / 2; /* 下一个扇区还是写不完 */
            }
            else
            {
