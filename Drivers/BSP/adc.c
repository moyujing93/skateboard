...
void adc1_dma_init(void)
{
    ...
    /* IO引脚配置 */
    gpio_init_struct.Pin   = MA_CUN_U_PIN | MA_CUN_V_PIN | MA_CUN_W_PIN | MB_CUN_U_PIN | MB_CUN_V_PIN | MB_CUN_W_PIN;
    gpio_init_struct.Mode  = GPIO_MODE_ANALOG;
    gpio_init_struct.Pull  = GPIO_NOPULL;
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(GPIOA, &gpio_init_struct);
    gpio_init_struct.Pin   = V_B_PIN | V_T_PIN;
    ...
    g_dma_adc_handle.Init.Mode = DMA_CIRCULAR;                                /* 循环模式选择 */
...
void adc_dma_conver(uint8_t num)
{
    ...
    I(A)=(U(mV)/20)/R(mR)
    adc = 4096 时 电压 3.3v  电压3.3v时 电流 3.3V/20倍/0.003R = 55A
    55A  =  4096 / x
    x = 4096 / 55A
    X = 0.074f
    */
     ...
     if(Get_time > 100)  /* 电压和温度变化没那么快，且耗时长，没必要太频繁计算， */
    {
        ...
        x   =  63.8v / 4096
        */
        g_bldc_motorA.v_bus = vb_temp * 15.57f;  //单位mV
